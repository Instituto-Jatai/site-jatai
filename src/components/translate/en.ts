const header = {
  Início: "Home",
  Equipe: "Team",
  Contato: "Contact",
  Casos: "Cases",
  "Pré-diagnóstico": "Pre-diagnosis",
  "NOVO!": "NEW!",
  "EM BREVE!": "COMING SOON!",
};

const footer = {
  "Vamos marcar uma conversa?": "Let's have a chat?",
  "Entre em contato para discutir conosco suas necessidades!":
    "Get in touch to discuss your needs with us! We can schedule an informal online meeting to think about how we can help you.",
  "Fale com a gente": "Contact us",
  "Siga a Jataí": "Follow Jataí",
};

const home = {
  "Construindo organizações públicas para o mundo <b>digital</b> e <b>sustentável</b>":
    "Building public organizations for the <b>digital</b> and <b>sustainable</b> world",
  "Somos uma organização sem fins lucrativos criada com a missão de apoiar organizações públicas a se tornarem mais ágeis para se adaptarem ao mundo digital e sustentável.":
    "We are a non-profit organization created with the mission to support public organizations in becoming more agile to adapt to the digital and sustainable world.",
  "Fale conosco": "Contact us",
  "Confira o edital para apoio a <b>contratação de sistema de gestão escolar</b>":
    "Check out the tender for support in the hiring of a school management system",
  "Qualquer secretaria, consórcio ou organização do terceiro setor que trabalhe com educação pode participar. Não deixe de conferir!":
    "Any government department, consortium, or third-sector organization working with education can participate. Don't miss out!",
  "Inscreva-se agora": "Sign up now",
  "Saiba mais": "Learn more",
  "O que fazemos": "What we do",
  "Ajudamos você que é gestor público a enfrentar os desafios da inovação e digitalização no setor público. Construímos com você soluções para desafios de equipe, governança, recursos e contratações.":
    "We help you, as a public manager, to face the challenges of innovation and digitization in the public sector. We work with you to build solutions for team, governance, resources, and contracting challenges.",
  "Construir equipes de tecnologia": "Building technology teams",
  "Apoiamos a criação de equipe interna e contratação de equipes.":
    "We support the creation of internal teams and hiring of external teams.",
  "<b>Por quê?</b> Sem equipes de tecnologia adequadas e qualificadas, iniciativas de uso de tecnologias não prosperam. Por isso, é importante garantir uma equipe com capacidade para transformar sua organização com uso de tecnologia.":
    "<b>Why?</b> Without adequate and qualified technology teams, technology initiatives do not prosper. Therefore, it is essential to ensure a team capable of transforming your organization using technology.",
  "<b>O que fazemos?</b> Apoiamos na definição de perfis necessários, da estrutura organizacional e na contratação de equipes multidisciplinares em modelo de Ateliê de software, para superar as dificuldades com as antigas contratações de fábrica de software":
    "<b>What we do?</b> We support defining the necessary profiles, organizational structure, and hiring of multidisciplinary teams in a software Atelier model, to overcome the challenges with traditional software factory contracts.",
  "<b>Resultados esperados:</b> Equipe estruturada e com capacidade de entrega de produtos digitais de qualidade mais rapidamente":
    "<b>Expected results:</b> A structured team capable of delivering high-quality digital products more quickly.",
  "Digitalizar gestão com uso de tecnologias abertas":
    "Digitizing management with the use of open technologies",
  "Apoiamos a escolha de tecnologias abertas que gerem impacto, eficiência e autonomia.":
    "We support the selection of open technologies that generate impact, efficiency, and autonomy.",
  "<b>Por quê?</b> Tecnologias abertas oferecem soluções comuns para necessidades compartilhadas entre diferentes organizações públicas. Mas além de entregarem resultados, geram mais eficiência nos gastos com tecnologias, garantem independência com relação aos fornecedores e permitem mais flexibilidade na entrega do serviço público.":
    "<b>Why?</b> Open technologies offer common solutions for shared needs among different public organizations. Besides delivering results, they generate more efficiency in technology expenses, ensure independence from suppliers, and provide more flexibility in delivering public services.",
  "<b>O que fazemos?</b> Apoiamos na busca de software em código-aberto que resolvam problemas de gestão pública, no levantamento do mercado de fornecedores para essas soluções e na contratação para implantação e fornecimento e suporte dessas soluções para a organização pública.":
    "<b>What we do?</b> We support the search for open-source software that solves public management problems, market research for suppliers of these solutions, and contracting for the implementation, provision, and support of these solutions for the public organization.",
  "<b>Resultados esperados:</b> Tecnologia aberta contratada e implantada, gerando resultados para a organização pública.":
    "<b>Expected results:</b> Open technology contracted and implemented, generating results for the public organization.",
  "Contratar soluções para problemas públicos":
    "Contract solutions for public problems",
  "Apoiamos na definição de problemas públicos e chamada de soluções para o ecossistema de inovação.":
    "We support the definition of public problems and the call for solutions from the innovation ecosystem.",
  "<b>Por quê?</b> Nem sempre a melhor solução para os problemas públicos sai de dentro das nossas organizações. Abrir problemas do governo para o ecossistema de inovação, permitindo o teste de soluções e sua contratação tem grande potencial para a resolução desses problemas de forma criativa.":
    "<b>Why?</b> Not always the best solution for public problems comes from within our organizations. Opening government problems to the innovation ecosystem, allowing the testing of solutions and their contracting has great potential for solving these problems in a creative way.",
  "<b>O que fazemos?</b> Apoiamos na definição de problemas públicos e na realização da chamada de soluções para o ecossistema de inovação. Estruturamos a chamada tendo como base a licitação prevista no Marco Legal de Startups e Empreendedorismo, o que permite o pagamento dos testes pela organização pública, assim como sua contratação caso esses testes sejam bem sucedidos.":
    "<b>What we do?</b> We assist in the definition of public problems and the call for solutions to the innovation ecosystem. We structure the call based on the tender foreseen in the Legal Framework for Startups and Entrepreneurship, which allows the public organization to pay for the tests, as well as their contracting if these tests are successful.",
  "<b>Resultados esperados:</b> Teste de soluções e contratação para a resolução do problema apresentado.":
    "<b>Expected results:</b> Testing of solutions and contracting for the resolution of the presented problem.",
  "Parceiros, Beneficiários e Clientes":
    "Partners, Beneficiaries, and Customers",
  "Edital para apoio a contratação de sistema de gestão escolar":
    "Announcement for support in the hiring of a school management system",
  "Este edital é focado para inscrições de secretarias de educação, consórcios públicos compostos por secretarias deeducação ou outras organizações sem fins lucrativos que apoiem secretarias de educação, interessadas em aprimorar políticas educacionais com uso de procedimentos de compras inovadoras. A atuação do Instituto Jataí nesta cooperação é total ou parcialmente financiada por apoio institucional recebido da Aliança entre Fundação Lemann e Imaginable Futures.":
    "This announcement is focused on applications from education departments, public consortiums made up of education departments or other non-profit organizations that support education departments, interested in improving educational policies with the use of innovative procurement procedures. The participation of the Jataí Institute in this cooperation is fully or partially funded by institutional support received from the Alliance between the Lemann Foundation and Imaginable Futures.",
};

const team = {
  "Equipe Jataí": "Jataí Team",
  "Diretor executivo": "Executive Director",
  "Doutorando em Política Científica e Tecnológica - Unicamp":
    "PhD Candidate in Science and Technology Policy - Unicamp",
  "Msc. Administração Pública - Columbia Graduado em Direito - USP":
    "Msc. Public Administration - Columbia Law Graduate - USP",
  "Gestora de projetos": "Project Manager",
  "Mestrado em Ciência Política - IESP-UERJ":
    "Master's in Political Science - IESP-UERJ",
  "Graduada em Relações Internacionais - UnB":
    "Graduate in International Relations - UnB",
  "Pesquisador e Analista júnior": "Researcher and Junior Analyst",
  "Mestrando em Política Científica Tecnológica – Unicamp":
    "Master's Candidate in Science Technology Policy – Unicamp",
  "Graduado em Direito - UFRJ": "Law Graduate - UFRJ",
  "Especialista em clima": "Climate Specialist",
  "Mestrado em Políticas Públicas - Oxford":
    "Master's in Public Policy - Oxford",
  "Graduada em Engenharia Ambiental - Unesp":
    "Environmental Engineering Graduate - Unesp",
  "Especialista em tecnologia": "Technology Specialist",
  "Graduada em Engenharia de Software - UnB":
    "Software Engineering Graduate - UnB",
};

const contact = {
  Contato: "Contact",
  "Envie um e-mail pra gente": "Send us an email",
  "Marque uma conversa no nosso Calendly": "Schedule a chat on our Calendly",
  "Entre em contato para discutir conosco suas necessidades!":
    "Get in touch to discuss your needs with us!",
  "Fale com a gente": "Talk to us",
};

const prediagnostic = {
  "Pré-diagnóstico": "Pre-diagnosis",
  "O que é o Pré-diagnóstico?": "What is the Pre-diagnosis?",
  "O pré-diagnóstico é uma ferramenta para redes de ensino que não possuem sistemas de gestão ou querem aprimorar seu uso. A partir das respostas, podemos identificar soluções compatíveis com o cenário em que sua secretaria se encontra e propor soluções conjuntas.":
    "Pre-diagnosis is a tool for educational networks that do not have management systems or want to improve their use. From the answers, we can identify solutions compatible with the scenario in which your secretariat is located and propose joint solutions.",
  "Acesse:": "Access:",
  "Iniciar preenchimento": "Start filling out",
};

const cases = {
  Casos: "Cases",
  "Tem dúvidas sobre como as compras públicas podem promover inovação e potencializar governos mais digitais e sustentáveis? Nós reunimos aqui casos diversos, com o objetivo de disseminar práticas inovadoras, promovendo o conhecimento, oportunidades de conexão e fortalecimento de redes.":
    "Do you have questions about how public procurement can promote innovation and potentiate more digital and sustainable governments? We have gathered various cases here, with the aim of disseminating innovative practices, promoting knowledge, connection opportunities, and strengthening networks.",
  "Como contratar tecnologias educacionais em escala?":
    "How to hire educational technologies on a large scale?",
  "Casos da Ata de Registro de Preços na Secretaria Estadual de Educação do Rio Grande do Sul":
    "Cases from the Price Record Sheet at the State Department of Education of Rio Grande do Sul",
  "A Secretaria de Educação do Rio Grande do Sul, em parceria com a Secretaria de Gestão, realizou a contratação de tecnologias para apoiar professores na diversificação das suas práticas pedagógicas em português e matemática. Este material tem como objetivo compartilhar principalmente os desafios e aprendizados das secretarias que realizaram pregão eletrônico de uma Ata de Registro de Preços para compra de recursos educacionais digitais, abrindo oportunidade para que não somente a rede estadual, mas também as redes municipais interessadas, pudessem contratar esses recursos.":
    "The Department of Education of Rio Grande do Sul, in partnership with the Department of Management, hired technologies to support teachers in diversifying their pedagogical practices in Portuguese and mathematics. The aim of this material is mainly to share the challenges and learnings of the departments that held an electronic auction of a Price Record Sheet for the purchase of digital educational resources, opening opportunities for not only the state network, but also the interested municipal networks, to hire these resources.",
  "Como melhorar resultados educacionais, otimizar recursos e gerenciar as rotinas da Secretaria de Educação?":
    "How to improve educational results, optimize resources and manage the routines of the Department of Education?",
  "O caso da secretaria de educação de Monte Alegre, no Rio Grande do Norte":
    "The case of the Department of Education of Monte Alegre, in Rio Grande do Norte",
  "A Secretaria Municipal de Educação de Monte Alegre, no Rio Grande do Norte, realizou a contratação de serviço de suporte para o uso do sistema i-Educar. Como o sistema tem licenciamento livre e está disponível para todos, a rede realizou a instalação por conta própria em 2014. Ao perceber a necessidade de aprofundamento do uso, decidiu pela contratação de um serviço de suporte ao sistema, com oferta de hospedagem, manutenção corretiva, suporte técnico e desenvolvimento de módulos específicos.":
    "The Municipal Department of Education of Monte Alegre, in Rio Grande do Norte, hired a support service for the use of the i-Educar system. As the system has free licensing and is available to everyone, the network carried out the installation on its own in 2014. Upon realizing the need for deeper usage, it decided to hire a system support service, offering hosting, corrective maintenance, technical support and development of specific modules.",
  "Como contratar recursos educacionais digitais (edtechs) para educação pública? Casos de uso do Marco Legal de Startups no programa impulsiONar":
    "How to contract digital educational resources (edtechs) for public education? Cases of use of the Legal Framework of Startups in the impulsiONar program",
  "O Programa ImpulsiONar tem como objetivo apoiar Secretarias Municipais de Educação no desenvolvimento de estratégias de prevenção e redução de defasagens em Língua Portuguesa e Matemática de estudantes do 6º ao 9º ano de sua rede de escolas públicas. A intervenção do programa nas Secretarias parte de um modelo pedagógico, que é colocado em prática nas redes por meio do apoio de organizações educacionais que oferecem serviços de implementação e formação de professores, além de ter a tecnologia (edtechs) como um recurso didático chave. Para fomentar o uso de tecnologia, o impulsiONar também apoia as Secretarias com os procedimentos jurídicos, financeiros e de gestão para testar a utilização de edtechs (recursos educacionais digitais). Três Secretarias Municipais de Educação realizaram, por meio do Marco Legal de Startups, a contratação de recursos educacionais digitais (edtechs) para apoiar professores e estudantes na evolução da aprendizagem de Matemática e Língua Portuguesa.":
    "The ImpulsiONar Program aims to support Municipal Departments of Education in the development of strategies to prevent and reduce lags in Portuguese Language and Mathematics of students from the 6th to the 9th grade of their network of public schools. The intervention of the program in the Departments is based on a pedagogical model, which is put into practice in the networks through the support of educational organizations that offer implementation services and teacher training, in addition to having technology (edtechs) as a key didactic resource. To promote the use of technology, ImpulsiONar also supports the Departments with legal, financial, and management procedures to test the use of edtechs (digital educational resources). Three Municipal Departments of Education carried out, through the Legal Framework of Startups, the hiring of digital educational resources (edtechs) to support teachers and students in the progress of learning Mathematics and Portuguese Language.",
  "Saiba mais": "Learn more",
};

const caseRS = {
  "A INOVAÇÃO": "THE INNOVATION",
  "Utilização de Ata de Registro de Preço":
    "Use of Price Registration Certificate",
  "Ata de Registro de Preço permite experimentar soluções e escalar de forma ágil sua implementação e traz eficiência aos processos de licitação pois permite que outros órgãos possam colaborar contratando conjuntamente a solução ou, dentro de limitações jurídicas, contratar a tecnologia cujo preço foi registrado em ata":
    "The Price Registration Certificate allows for experimenting solutions and scaling its implementation agilely, and brings efficiency to bidding processes as it allows other bodies to collaborate by jointly contracting the solution or, within legal limitations, contracting the technology whose price was recorded in the certificate.",
  "Órgãos públicos que participaram": "Public agencies that participated",
  "Secretaria Estadual de Educação (Seduc) do Rio Grande do Sul":
    "State Department of Education (Seduc) of Rio Grande do Sul",
  "Secretaria de Planejamento, Governança e Gestão (SPGG) do Rio Grande do Sul":
    "Department of Planning, Governance and Management (SPGG) of Rio Grande do Sul",
  "Principais desafios": "Main challenges",
  "A emergência trazida pela pandemia fez com o que o processo tivesse que ser realizado em tempo recorde.":
    "The emergency brought by the pandemic required the process to be carried out in a record time.",
  "Comprar algo pronto, sem a necessidade de desenvolvimento do produto.":
    "Buy something ready, without the need for product development.",
  "Principais aprendizados": "Main learnings",
  "Modelo de licenciamento poderia ser aprimorado para prever pagamento por licenças em uso e/ou também faixas de preço para se beneficiar dos ganhos de escala no fornecimento da tecnologia":
    "The licensing model could be improved to provide for payment for licenses in use and/or also price ranges to benefit from scale gains in technology supply.",
  "Realização de Prova de Conceito é importante para garantir que a solução vencedora na etapa de lances por menor preço do pregão esteja de acordo com as necessidades especificadas":
    "Carrying out a Proof of Concept is important to ensure that the winning solution in the step of bids for the lowest auction price meets the specified needs.",
  "Principais Resultados": "Main Results",
};
const caseMonteAlegre = {
  "Como melhorar resultados educacionais, otimizar recursos e gerenciar as rotinas da Secretaria de Educação? O caso da secretaria de educação de Monte Alegre, no Rio Grande do Norte":
    "How to improve educational outcomes, optimize resources and manage routines of the Department of Education? The case of the Department of Education of Monte Alegre, in Rio Grande do Norte",
  "A Secretaria Municipal de Educação de Monte Alegre, no Rio Grande do Norte, realizou a contratação de serviço de suporte para o uso do sistema i-Educar. Como o sistema tem licenciamento livre e está disponível para todos, a rede realizou a instalação por conta própria em 2014. Ao perceber a necessidade de aprofundamento do uso, decidiu pela contratação de um serviço de suporte ao sistema, com oferta de hospedagem, manutenção corretiva, suporte técnico e desenvolvimento de módulos específicos.":
    "The Department of Education of Monte Alegre, in Rio Grande do Norte, hired a support service for the use of the i-Educar system. As the system is open-source and available to everyone, the network carried out the installation on its own in 2014. Recognizing the need for deeper use, it decided to hire a support service for the system, offering hosting, corrective maintenance, technical support and development of specific modules.",
  "A INOVAÇÃO": "THE INNOVATION",
  "Utilização de sistema de código aberto com contratação de serviço de suporte":
    "Use of open-source system with contracted support service",
  "A implantação de sistema de código aberto, como o i-Educar, permite autonomia para a gestão municipal, uma vez que afasta a dependência de único fornecedor e garante que a estrutura e dados do sistema pertençam à Secretaria. A contratação do serviço de suporte, após a implementação inicial do sistema, foi uma inflexão importante na gestão, ao trazer para o órgão facilidade na manutenção do sistema e no suporte às escolas.":
    "The implementation of an open-source system, like i-Educar, allows autonomy for municipal management, since it eliminates the dependence on a single supplier and ensures that the system's structure and data belong to the Department. Hiring the support service, after the system's initial implementation, was a significant inflection point in management, bringing ease in maintaining the system and supporting schools to the department.",
  "Órgãos públicos que participaram": "Public agencies that participated",
  "Secretaria Municipal de Educação": "Municipal Department of Education",
  "Secretaria Municipal de Finanças": "Municipal Department of Finance",
  "Principais desafios": "Main challenges",
  "Um desafio foi a decisão pela contratação do serviço, uma vez que a rede havia iniciado a implementação com uso de recursos próprios, tendo sido preciso analisar prós e contras.":
    "One challenge was the decision to hire the service, as the network had started the implementation using its own resources, making it necessary to weigh pros and cons.",
  "Outro desafio central foi a implantação do projeto junto às escolas e professores, uma vez que representava mudança nas formas de trabalho.":
    "Another central challenge was the implementation of the project along with schools and teachers, as it represented a change in work methods.",
  "Principais aprendizados": "Main learnings",
  "A contratação de serviço de suporte para o sistema de gestão escolar de código aberto otimiza as rotinas, uma vez que o serviço é especialista na temática, mas é essencial que o sistema seja estudado antes pela Secretaria e que se produzam decisões de uso pelo órgão que levem a seu uso efetivo. Isto é, o preenchimento de informações e a análise delas deve compor a rotina de trabalho das Secretarias para que o sistema alavanque bons resultados.":
    "Hiring support service for the open-source school management system optimizes routines, since the service is expert in the subject, but it is essential that the system be studied first by the Department and that usage decisions are made by the agency leading to its effective use. That is, filling in information and analyzing it should be part of the Secretaries' work routine in order for the system to leverage good results.",
  "Principais Resultados": "Main Results",
  "A melhoria da gestão educacional gerou economia de R$2,4 milhões por ano, segundo cálculos da Secretaria, a partir de ampliação da arrecadação do Fundeb em 8%, com a comprovação de alunos matriculados na rede, do aumento de 61% para 95% na ocupação das escolas, permitindo reduzir o número de professores contratado, e da economia de 15% da merenda escolar. A rede pode, então, alocar os recursos em outras necessidades elencadas.":
    "The improvement of educational management saved R$2.4 million per year, according to the Department's calculations, due to an increase in Fundeb revenue by 8%, with the proof of students enrolled in the network, an increase from 61% to 95% in school occupancy, allowing to reduce the number of hired teachers, and a saving of 15% of school meals. The network was then able to allocate resources to other listed needs.",
};

const caseImpulsionar = {
  "Como contratar recursos educacionais digitais (edtechs) para educação pública? Casos de uso do Marco Legal de Startups no programa impulsiONar":
    "How to hire digital educational resources (edtechs) for public education? Use cases of the Legal Framework for Startups in the impulsiONar program",
  "O Programa ImpulsiONar tem como objetivo apoiar Secretarias Municipais de Educação no desenvolvimento de estratégias de prevenção e redução de defasagens em Língua Portuguesa e Matemática de estudantes do 6º ao 9º ano de sua rede de escolas públicas. A intervenção do programa nas Secretarias parte de um modelo pedagógico, que é colocado em prática nas redes por meio do apoio de organizações educacionais que oferecem serviços de implementação e formação de professores, além de ter a tecnologia (edtechs) como um recurso didático chave. Para fomentar o uso de tecnologia, o impulsiONar também apoia as Secretarias com os procedimentos jurídicos, financeiros e de gestão para testar a utilização de edtechs (recursos educacionais digitais). Três Secretarias Municipais de Educação realizaram, por meio do Marco Legal de Startups, a contratação de recursos educacionais digitais (edtechs) para apoiar professores e estudantes na evolução da aprendizagem de Matemática e Língua Portuguesa.":
    "The ImpulsiONar Program aims to support Municipal Education Departments in the development of strategies for the prevention and reduction of deficits in Portuguese Language and Mathematics for students from the 6th to the 9th grade of their public school network. The program's intervention in the Departments is based on a pedagogical model, which is put into practice in the networks through the support of educational organizations that offer implementation and teacher training services, in addition to having technology (edtechs) as a key didactic resource. To foster the use of technology, impulsiONar also supports the Departments with legal, financial, and management procedures to test the use of edtechs (digital educational resources). Three Municipal Education Departments carried out, through the Legal Framework for Startups, the hiring of digital educational resources (edtechs) to support teachers and students in the evolution of learning in Mathematics and Portuguese Language.",
  "A INOVAÇÃO": "THE INNOVATION",
  "Uso do MLS no setor da educação para contratação de edtechs":
    "Use of MLS in the education sector for hiring edtechs",
  "A inovação do programa foi a realização do processo de inovação aberta para contratação de teste de edtechs pelo setor público (Secretarias Municipais de Educação), via Marco Legal das Startups (MLS). O arranjo permitiu o teste remunerado para edtechs nas Secretarias de Educação Municipal (ainda que sem transferência de recursos públicos e sim via terceiro setor) - e há a possibilidade de posterior contratação direta pela administração pública.":
    "The innovation of the program was the accomplishment of the open innovation process for contracting the testing of edtechs by the public sector (Municipal Education Departments), via Legal Framework for Startups (MLS). The arrangement allowed paid testing for edtechs in the Municipal Education Departments (even without transfer of public resources and rather via the third sector) - and there is the possibility of later direct contracting by the public administration.",
  "Órgãos públicos que participaram do programa e utilizaram o MLS":
    "Public agencies that participated in the program and used MLS",
  "Secretaria Municipal de Educação de Igarassu (Pernambuco)":
    "Municipal Department of Education of Igarassu (Pernambuco)",
  "Secretaria Municipal de Educação de Guaramiranga (Ceará)":
    "Municipal Department of Education of Guaramiranga (Ceará)",
  "Secretaria Municipal de Educação de Domingos Mourão (Piauí)":
    "Municipal Department of Education of Domingos Mourão (Piauí)",
  "Principais desafios": "Main challenges",
  "Sendo a utilização do Marco Legal das Startups na área de educação algo inédito no Brasil, houve diversos desafios, aprendizados e boas práticas ao longo da jornada do programa":
    "As the use of the Legal Framework for Startups in the field of education is something unprecedented in Brazil, there have been several challenges, learnings, and good practices throughout the program's journey",
  "Principais aprendizados": "Main learnings",
  "A construção das metas configura-se como um risco a ser mapeado, uma vez que essas são desenhadas dentro de um cenário de incertezas característico do modelo de desafio. Por isso, essas devem ser feitas com base no maior número de evidências contextuais, mas sendo importante acompanhá-las ao longo do processo e calibrá-las sempre que possível e necessário.":
    "The construction of the goals is seen as a risk to be mapped, since they are designed within a scenario of uncertainties characteristic of the challenge model. Therefore, they must be made based on the greatest number of contextual evidence, but it is important to follow them throughout the process and calibrate them whenever possible and necessary.",
  "Ainda que a contratação trate de um desafio específico, é importante mapear situações correlatas ao desafio - como, no caso do Impulsionar, foi a situação da infraestrutura (conectividade e equipamentos), descrita no item abaixo.":
    "Even though the hiring deals with a specific challenge, it is important to map situations related to the challenge - as was the case with Impulsionar, which was the infrastructure situation (connectivity and equipment), described in the item below.",
  "Principais resultados": "Main results",
  "Foram definidas 4 metas para o 1º período de teste das edtechs (de Abril à Dezembro de 2022), relacionadas a:":
    "4 goals were set for the 1st test period of the edtechs (from April to December 2022), related to:",
  "Implementação nas escolas;": "Implementation in schools;",
  "Formação dos usuários;": "Training of users;",
  "Acesso e utilização dos estudantes;": "Access and use by students;",
  "Essa são as métricas para avaliar se o teste foi bem sucedido - e que então possibilitam a contratação das edtechs pelas Secretarias.":
    "These are the metrics to evaluate whether the test was successful - and that then enable the hiring of edtechs by the Departments.",
  "Os resultados obtidos foram (1) implementação em 100% das escolas, (2) formação de mais de 80% dos usuários e alta satisfação (> 80%) inicial dos gestores e professores. Porém, a meta (3) de acesso e utilização dos estudantes ficou bastante abaixo do esperado (entre 50-70% de utilização em média, a depender do mês e da rede, versus a meta que seria 80% dos estudantes utilizando 1 vez ao mês). Isso se deu devido a falta de infraestrutura (conectividade e equipamentos) que permitisse o amplo uso dos estudantes.":
    "The results obtained were (1) implementation in 100% of the schools, (2) training of more than 80% of the users and high initial satisfaction (> 80%) of managers and teachers. However, the goal (3) of access and use by students was well below expected (between 50-70% use on average, depending on the month and the network, versus the goal which would be 80% of students using it once a month). This was due to a lack of infrastructure (connectivity and equipment) that would allow widespread use by students.",
  "Dado então os desafios de tecnologia, o programa enviou equipamentos para as redes de ensino e período do piloto foi estendido, ofertando para as redes de ensino mais 6 meses (de jan-jun/23), para permitir o uso intensivo da solução - e os resultados concretos em termos de evolução da aprendizagem dos estudantes":
    "Given the technology challenges, the program sent equipment to the school networks and the pilot period was extended, offering the school networks an additional 6 months (from Jan-Jun/23), to allow intensive use of the solution - and concrete results in terms of students' learning progress",
};

const modelDocs = {
  "Documentos modelo": "Model Documents",
  "Modelo ETP e TR": "ETP and TR Model",
  "Para contratação de sistema de gestão escolar":
    "For contracting a school management system",
  "Os modelos de Estudo Técnico Preliminar (ETP) e Termo de Referência (TR) para a contratação de i-Educar e i-Diário estão prontos para serem utilizados por qualquer Secretaria de Educação interessada em informatizar a gestão escolar. Esses sistemas são de código aberto e possuem licenciamento gratuito, o que significa que o foco da contratação está no serviço de sustentação do sistema, isto é, a implementação dos sistemas, a migração de dados, o treinamento dos servidores, além da hospedagem em nuvem, suporte técnico e manutenção corretiva e evolutiva. É importante ressaltar que os modelos foram desenvolvidos considerando o acesso público ao código-fonte, visando estimular a ampla concorrência e garantir transparência no processo. Os modelos são para contratação em formato pregão e, a fim de promover ainda mais a participação e a transparência, recomendamos a realização de uma consulta pública antes da contratação. Estamos à disposição caso precisem de ajuda nesse processo, é só agendar uma conversa aqui pelo site!":
    "The Preliminary Technical Study (ETP) and Reference Term (TR) models for contracting i-Educar and i-Diário are ready to be used by any Education Secretariat interested in computerizing school management. These systems are open source and have free licensing, which means that the focus of the contract is on system support service, that is, system implementation, data migration, server training, in addition to cloud hosting, technical support and corrective and evolutionary maintenance. It is important to note that the models were developed considering public access to the source code, aiming to stimulate broad competition and ensure transparency in the process. The models are for contracting in bidding format and, in order to promote even more participation and transparency, we recommend conducting a public consultation before contracting. We are available if you need help in this process, just schedule a conversation here on the website!",
  "Para montar equipes de tecnologia": "To assemble technology teams",
  "Complete o formulário e faça o download gratuito do documentos modelo":
    "Complete the form and download the model documents for free",
  Nome: "Name",
  "E-mail": "E-mail",
  "Orgão Público": "Public Agency",
  "Os documentos foram baixados com sucesso.":
    "The documents were successfully downloaded.",
  "Por favor, verifique a pasta de downloads do seu computador para acessá-los.":
    "Please check your computer's download folder to access them.",
  "Os modelos de <b>Estudo Técnico Preliminar (ETP)</b> e <b>Termo de Referência (TR)</b> para contratação de Ateliê de Software estão prontos para serem utilizados por qualquer organização pública interessada em terceirização de equipes de tecnologia. A terceirização de equipes no modelo de Ateliê de Software foca na contratação de equipes multidisciplinares para desenvolvimento de produtos digitais utilizando métodos ágeis e design centrado no usuário.":
    "The models of the <b>Preliminary Technical Study (PTS)</b> and <b>Terms of Reference (TOR)</b> for hiring Software Atelier are ready to be used by any public organization interested in outsourcing technology teams. The outsourcing of teams in the Software Atelier model focuses on hiring multidisciplinary teams for the development of digital products using agile methods and user-centered design.",
};

export default {
  ...header,
  ...footer,
  ...home,
  ...team,
  ...contact,
  ...prediagnostic,
  ...cases,
  ...caseRS,
  ...caseMonteAlegre,
  ...caseImpulsionar,
  ...modelDocs,
};
